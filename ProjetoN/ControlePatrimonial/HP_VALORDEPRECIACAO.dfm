object H_FormaCalcDep: TH_FormaCalcDep
  Left = 0
  Top = 0
  Caption = 'Help Valor Deprecia'#231#227'o'
  ClientHeight = 257
  ClientWidth = 428
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  OldCreateOrder = False
  PixelsPerInch = 96
  TextHeight = 13
  object Memo1: TMemo
    Left = 0
    Top = 0
    Width = 428
    Height = 216
    Align = alClient
    Enabled = False
    Lines.Strings = (
      'VALOR DE DEPRECIA'#199#195'O - FORMA DE C'#193'LCULO'
      ''
      'R$ 58.000,00 '#8211' Aquisi'#231#227'o do Produto '
      'R$ 2.000,00 - Valor Agregado (manuten'#231#227'o)'
      'R$ 20.000,00 '#8211' Residual '
      
        'R$ 40.000,00 '#8211' Base de C'#225'lculo da Deprecia'#231#227'o ( 60.000,00 - 20.0' +
        '00,00)'
      '25% '#8211' Taxa de deprecia'#231#227'o anual '
      'R$ 10.000,00 '#8211' Deprecia'#231#227'o Anual (40.000,00 x 25% = 10.000,00) '
      
        'R$ 833,33 '#8211' Deprecia'#231#227'o mensal a ser reconhecida contabilmente (' +
        '10.000,00 / 12 = '
      '833,33)')
    TabOrder = 0
    ExplicitLeft = 88
    ExplicitTop = 8
    ExplicitWidth = 185
    ExplicitHeight = 89
  end
  object Panel1: TPanel
    Left = 0
    Top = 216
    Width = 428
    Height = 41
    Align = alBottom
    TabOrder = 1
    ExplicitLeft = 120
    ExplicitTop = 136
    ExplicitWidth = 185
    object sbSair: TSpeedButton
      Left = 331
      Top = 1
      Width = 96
      Height = 39
      Align = alRight
      Caption = 'Sair'
      Glyph.Data = {
        
        EDEDEDEDEDEDEDEDEDEDEDEDEDEDEDED0000}
      OnClick = sbSairClick
      ExplicitLeft = 670
      ExplicitHeight = 40
    end
  end
end
